 ...
 * Settings class
 */
if ( !class_exists( 'WooCommerce_Delivery_Notes_Settings2' ) ) {

	class WooCommerce_Delivery_Notes_Settings2 {
		...
		public function add_settings_tab( $settings_tabs ) {
			$settings_tabs[$this->tab_name] = __( 'Print 2', 'woocommerce-delivery-notes' );
		...
		public function get_settings() {			
		    $settings = array(
				...
				array( 
			        'title' => __( 'Fields', 'woocommerce-delivery-notes' ), 
			        'type'  => 'title', 
			        'desc'  => '', 
			        'id'    => 'fields_options' 
		        ),
				...
				array(
					...
					'desc'     => __( 'The next invoice number.', 'woocommerce-delivery-notes' ),
				...
				array(
					...
					'desc'     => __( 'This text will be prepended to the invoice number.', 'woocommerce-delivery-notes' ),
				...
				array(
					...
					'desc'     => __( 'This text will be appended to the invoice number.', 'woocommerce-delivery-notes' ),
			...
			if( $position != false ) {
				...
				for( $i = $length - 1; $i >= 0; $i-- ) {
					...
					if( $i == 0 ) {
						...
						$desc_tip = __( 'The print buttons are available in the orders and on the order details screen.', 'woocommerce-delivery-notes' );
						$checkboxgroup = 'end';
					}
		...
		public function get_template_description() {		
			...
			if( $query->have_posts() ) {
				...
				$description = sprintf( __( 'You can preview the <a href="%1$s" target="%4$s" class="%5$s">invoice</a>, <a href="%2$s" target="%4$s" class="%5$s">delivery note</a> or <a href="%3$s" target="%4$s" class="%5$s">receipt</a> template.', 'woocommerce-delivery-notes' ), $invoice_url, $delivery_note_url, $receipt_url, '_blank', '' ); 
			}
			...
			?><tr valign="top">
				...
				<td class="forminp image_width_settings">
					...
					<div id="<?php echo esc_attr( $value['id'] ); ?>_field" class="<?php echo $class_name; ?>-field <?php echo esc_attr( $value['class'] ); ?>" style="<?php echo esc_attr( $value['css'] ); ?>">
						...
						<div id="<?php echo esc_attr( $value['id'] ); ?>_placeholder" class="<?php echo $class_name; ?>-placeholder <?php echo esc_attr( $value['class'] ); ?>">
							<div class="thumbnail">
